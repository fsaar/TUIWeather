...
class WeatherInfoCityTests: XCTestCase {
    ...
    func testThatItShouldCorrectlyDecodeGivenData() {
        ...
        let weatherInfoCity = try! WeatherClient.jsonDecoder.decode(WeatherInfoCity.self,from: weatherInfoCityData)
    ...
    func testThatItShouldNotThrowGivenInvalidData() {
        ...
        let weatherInfoCityData = NSData(contentsOfFile: path)! as Data
        XCTAssertNoThrow({
            _ = try! WeatherClient.jsonDecoder.decode(WeatherInfo.self,from: weatherInfoCityData)
        })
    }
}
